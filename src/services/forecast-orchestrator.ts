...
import { OpenMeteoForecastProvider } from '@/providers/open-meteo-forecast';
...
export class ForecastOrchestrator {
  private readonly nwsProvider = new NwsProvider();
  private readonly globalProvider = new OpenMeteoForecastProvider();

  async get(location: CanonicalLocation): Promise<ForecastResult> {
    const key = `${location.latitude.toFixed(4)}:${location.longitude.toFixed(4)}`;
    const existing = cache.get(key);
    if (existing && existing.expiresAt > Date.now()) return existing.value;
    const value = location.countryCode === 'US'
      ? await this.nwsProvider.getForecast(location)
      : await this.globalProvider.getForecast(location);
